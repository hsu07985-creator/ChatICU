...
TASK_PROMPTS: dict[str, str] = {
    ...
    "clinical_polish": (
        ...
        "DRUG NAME VERIFICATION: when the draft mentions a drug name, scan patient.medications "
        "case-insensitively against name / generic / brand fields. Three cases: "
        "(a) Drug found — proceed normally. "
        "(b) Drug name visually similar to an active drug but not an exact match (e.g. "
        "'Cefazoline' vs 'Cefazolin', 'Vancomycine' vs 'Vancomycin') — at the TOP of the "
        "polished output prepend a single line: "
        "'⚠️ 藥名待確認：草稿寫「<as-typed>」，建議確認是否為「<active drug name>」' "
        "then continue with the polished text. "
        "(c) Drug name does not match any active medication — at the TOP prepend: "
        "'⚠️ 用藥未在病人現用清單：「<drug>」。可能為新增建議或已停用藥物，請確認後再採用。' "
        "then continue. At most ONE drug-verification warning per output (the most clinically "
        "significant). NEVER block — always still produce the polished text below the warning. "
        ...
        # === GERIATRIC SAFETY CHECK (Beers Criteria minimal) ===
        "GERIATRIC SAFETY CHECK: if patient.age >= 65, AND the draft or polished output "
        "mentions any of: NSAIDs (ibuprofen, naproxen, ketorolac, etoricoxib), long-acting "
        "benzodiazepines (diazepam, clonazepam), first-generation antihistamines "
        "(diphenhydramine, hydroxyzine), sliding-scale insulin as sole therapy, dual "
        "antiplatelet (ASA + clopidogrel/ticagrelor without clear indication), or skeletal "
        "muscle relaxants (baclofen, methocarbamol, cyclobenzaprine) — append AT THE END of "
        "the polished text: "
        "'⚠️ 高齡用藥提醒：<class> 對 ≥65 歲列為 Beers 不建議用藥，請評估必要性與替代方案。' "
        "At most ONE geriatric warning per output. "
        # === RENAL DOSE HELPER ===
        "RENAL DOSE HELPER: if the draft mentions kidney function adjustment (keywords "
        "'CrCl', 'creatinine', 'renal', '腎功能', '劑量調整', 'dose adjust') AND "
        "patient.age, patient.weight, patient.gender, and a recent serum creatinine "
        "(patient.lab_data.creatinine or equivalent) are all present, annotate the relevant "
        "sentence with '（Cockcroft-Gault 估算 CrCl ≈ <X> mL/min）'. Formula: "
        "((140 - age) × weight_kg) / (72 × SCr_mg/dL), multiplied by 0.85 if female. Round "
        "to integer. If any input is missing, do NOT calculate or annotate. "
