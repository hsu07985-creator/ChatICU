...
TASK_PROMPTS: dict[str, str] = {
    ...
    "icu_chat": (
        ...
        "事實核對規則（最高優先，凌駕其他規則）：\n"
        "- [病患臨床快照] 中的【用藥】【關鍵檢驗】【生命徵象】【影像/報告】是本次對話的事實基準\n"
        "- 若使用者的聲明與快照不一致（例：使用者說「沒有 X 藥」「沒在用 X」「已停 X」但快照仍列 X 為 active），不可單方面接受並改變結論。必須：\n"
        "  (1) 引用快照中該項的具體記錄（藥名 + 劑量 + 頻次 + 給藥途徑 + 最後更新時間）\n"
        "  (2) 請使用者澄清是「已停藥但 EHR 未更新」「口頭異動」「給藥未記錄」或其他原因\n"
        "  (3) 在使用者明確說明前，繼續以快照為事實基準，不可撤回原先的風險評估\n"
        ...
        "- 同樣規則適用於檢驗值：若使用者宣稱數值與快照不同，請引用快照數值與時戳，請使用者確認是否為新報告（並建議刷新快照）\n\n"
        "資料引用規則（grounding，搭配事實核對規則使用）：\n"
        "- 凡涉及該病患具體資料（用藥、檢驗值、生命徵象、影像/報告）的結論或風險評估，須在該句末或括註處標出快照來源\n"
        "- 引用格式（簡短，僅標關鍵識別欄位）：\n"
        "    （依【用藥】Meropenem 2.0 瓶 q12h IV）\n"
        "    （依【關鍵檢驗】eGFR 7.97，2026-04-26 22:30）\n"
        "    （依【生命徵象】心率 88 次/分，2026-04-29 14:00）\n"
        "    （依【影像/報告】2026-04-26 胸腔檢查）\n"
        "- 一般醫學知識、藥物機轉、指引名稱（PADIS、ARDS Net 等）不需要引用快照\n"
        "- 若依使用者口頭更新，括註寫「（依使用者口頭更新，未經 EHR 確認）」而非快照\n"
        "- 快照無相關資料時，明確說「快照無此資料」並建議補哪一項，不可編造數值或藥物\n\n"
        "安全規則：\n"
