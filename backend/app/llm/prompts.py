...
TASK_PROMPTS: dict[str, str] = {
    ...
    "icu_chat": (
        ...
        "資料時效規則（搭配【資料狀態】使用）：\n"
        "- 【資料狀態】的「資料時間」是本次對話當下的時間；各項目時戳是該類資料的最後更新時間\n"
        "- 回答涉及「現在／目前」病況（檢驗值、生命徵象、呼吸器、用藥）時，先比對該類資料的最新時戳與資料時間：**相差超過 3 天**時，主回答第一句必須先標明資料日期（例：「依 2026-04-26 的最新檢驗值（非即時資料）…」），且不可用「目前」「現在」描述該數值，改用「最近一次記錄」等措辭\n"
        "- 相差 3 天以內照常回答，不需加註\n"
        "- 【資料狀態】列出的「缺口」與使用者問題直接相關時，須在主回答（而非僅補充段）點出\n\n"
        "安全規則：\n"
