...
Locks in the three regressions that the helper exists to prevent:
  1. icu_chat streaming MUST skip reasoning even when LLM_REASONING_EFFORT is
     set (TTFT carve-out).
  2. gpt-5.x without reasoning MUST send reasoning_effort="none" — never
     temperature — or the server defaults to medium and burns the entire
     completion budget on reasoning tokens. (Pre-5.5 gpt-5 used "minimal";
     gpt-5.5+ replaced that slot with "none" and 400s on "minimal".)
...
def test_streaming_icu_chat_on_gpt5_uses_none_even_when_reasoning_set(monkeypatch):
    ...
    assert block == {"reasoning_effort": "none"}
...
def test_call_openai_disable_reasoning_on_gpt5_falls_back_to_none(monkeypatch):
    _reload_with(monkeypatch, model="gpt-5.4-mini", reasoning_effort="low")
    block = llm_module._build_openai_reasoning_param_block(
    ...
    # an explicit "none" (gpt-5.5+ rejects "minimal").
    assert block == {"reasoning_effort": "none"}
...
def test_call_openai_multi_on_gpt5_with_empty_reasoning_uses_none(monkeypatch):
