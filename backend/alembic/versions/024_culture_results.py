    ...
    for pid, sheet, spec, spec_code, dept, col_at, rep_at, iso, susc in SEED_CULTURES:
        op.execute(
            t.insert().values(
                ...
                collected_at=col_at,
                reported_at=rep_at,
                isolates=json.dumps(iso),
                susceptibility=json.dumps(susc),
            )
        )
