...
| TC-W1-T3 | `roleDisplayName` 補 `np` + 改 `Record<UserRole, string>` | F-08 | `src/pages/chat.tsx:37-42`、`src/components/ui/mention-textarea.tsx:6-12`（共用 ROLE_LABEL） | 建立 NP 帳號發訊 → 顯示「專科護理師」而非 `np`；TS 編譯確認 enum 完整 | ☐ |
