...
| **B. Prefetch miss-rate 過 15%** | 加 structured log `[CHAT][PREFETCH][MISS]`：當沒 prefetch 觸發但 LLM 回應出現「缺少…」「若有…可進一步」等 hedging 字眼 | 加 1h log 程式碼 + 觀察 2 週 |
...
| 優先 | 動作 | 工時 | 為何 |
|-----|------|------|------|
| 🔴 1 | **V1 prod 5-case 真人測試** | 30-60 min | 沒這個 data 整個 F4 決策都是猜的 |
| 🔴 2 | **V2 權限 4-case 真人測試** | 20-30 min | 個資洩露 hard fail 必先排 |
| 🟡 3 | **加 prefetch miss-rate metric** | 1-1.5h | 自動收集訊號 B；2 週後就有 data 決策 |
