...
> **2026-07-23 產品決策（取代本文「模式 2：病人頁嵌入」建議）**：PAD 藥物劑量計算只保留於獨立「藥事工具 → 劑量計算」頁（`/pharmacy/dosage`），**病人詳情的「用藥」頁不顯示 PAD 計算器**。共用元件與後端 API 保留，但不從 `PatientMedicationsTab` 掛載。已於 commit `c7da5837a` 實作並部署 production。
