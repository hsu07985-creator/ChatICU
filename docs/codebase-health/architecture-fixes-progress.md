...
- **B3 後續**:✅ **Depends 化完成(2026-07-20)**——`app/dependencies.py::get_accessible_patient` 取代 13 處手刻 fetch+404+verify(10 個 router);scores.py 的 layer2 fallback 為唯一documented例外;靜態守衛測試禁止 imperative 樣式回流。**剩** ai_chat/clinical SSE 編排下沉(綁 Request/StreamingResponse,獨立批次)。
