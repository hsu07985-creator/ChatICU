...
- **D1 後續**:6 個凍結頁已拆 1(chat.tsx,2026-07-20 live 驗證發送流一致);已拆 5(+ai-chat→抽 `ai-chat-utils.ts` 純轉換/型別,938→799);剩 patient-detail(孤兒清理後 1502)。(附帶發現:tsconfig 無 noUnusedLocals + eslint no-unused-vars off,兩者都不抓孤兒 import——前三頁殘留已清並補進 gate。)
- **C5**:staging 決策。
