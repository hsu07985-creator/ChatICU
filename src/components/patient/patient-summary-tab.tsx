    ...
    <div className="space-y-3">
      <Card className="overflow-hidden border border-slate-200 bg-gradient-to-br from-slate-50 via-white to-slate-100/80">
        <CardHeader className="border-b border-slate-200/80 bg-white/70 pb-2.5">
          <CardTitle className="text-xl font-bold tracking-tight text-slate-900">基本資訊 / 症狀 / 入院診斷</CardTitle>
        ...
        <CardContent className="space-y-3 pt-3">
          ...
          <section className="rounded-md border border-slate-200 bg-white p-3">
            ...
            >
              <div className="p-3">
                <p className="text-sm font-semibold tracking-wide text-slate-700">症狀 Symptom</p>
                {symptoms.length > 0 ? (
                  <ul className="mt-1.5 overflow-hidden rounded-md border border-slate-200 bg-slate-50/60">
                    {symptoms.map((symptom: string, idx: number) => (
                      <li
                        key={idx}
                        className="border-b border-slate-200 px-3 py-2 text-base leading-snug text-slate-800 last:border-b-0"
                      >
                        {symptom}
                      </li>
                    ))}
                  ...
                  <p className="mt-1.5 text-base text-muted-foreground">尚無症狀記錄</p>
                )}
              </div>
              <div className="border-l border-slate-200 bg-blue-50/40 p-3">
                <p className="text-sm font-semibold tracking-wide text-slate-700">入院診斷</p>
                <div className="mt-1.5 rounded-md border border-blue-200/80 bg-white px-3 py-2">
                  <p className="text-base font-semibold leading-snug text-slate-900">{patient.diagnosis || '-'}</p>
                </div>
              </div>
            </div>
          </section>
