    ...
    return (
      <div className="rounded-lg border-2 border-green-400 bg-green-50 px-4 py-3 space-y-1">
        <div className="flex items-center gap-2 text-green-800 font-semibold text-sm">
          ...
          <span>{t('ivCompat.result.compatible')}</span>
        </div>
        <p className="text-xs text-green-700">
          <span className="font-medium">{drug1}</span>
          {' '}&{' '}
          <span className="font-medium">{drug2}</span>
          {' '}{t('ivCompat.result.ySite')}
        </p>
        {timeStability && (
          <p className="text-xs text-green-600">{t('ivCompat.result.stability', { time: timeStability })}</p>
        )}
        {notes && <p className="text-xs text-green-600">{notes}</p>}
        {references && <p className="text-xs text-green-500">{t('ivCompat.result.referencePrefix', { ref: references })}</p>}
      </div>
    );
  }
    ...
    return (
      <div className="rounded-lg border-2 border-red-500 bg-red-50 px-4 py-3 space-y-1.5">
        <div className="flex items-center gap-2 text-red-800 font-bold text-sm">
          ...
          <span>{t('ivCompat.result.incompatible')}</span>
        </div>
        <Alert variant="destructive" className="py-2 border-red-400">
          <AlertTriangle className="h-4 w-4" />
          <AlertTitle className="text-sm font-bold">
            {t('ivCompat.result.alertTitle')}
          </AlertTitle>
          <AlertDescription className="text-xs mt-0.5">
            {t('ivCompat.result.incompatibleDescription', { a: drug1, b: drug2 })}
          </AlertDescription>
        </Alert>
        {notes && <p className="text-xs text-red-700 mt-1">{notes}</p>}
        {references && <p className="text-xs text-red-500">{t('ivCompat.result.referencePrefix', { ref: references })}</p>}
      </div>
    );
  }
  ...
  return (
    <div className="rounded-lg border border-gray-300 dark:border-slate-600 bg-gray-50 dark:bg-slate-800 px-4 py-3 space-y-1">
      <div className="flex items-center gap-2 text-gray-600 dark:text-gray-400 font-medium text-sm">
        ...
        <span>{t('ivCompat.result.noData')}</span>
      </div>
      <p className="text-xs text-gray-500 dark:text-gray-400">
        {t('ivCompat.result.noDataDescription', { a: drug1, b: drug2 })}
      </p>
    </div>
  );
}
      ...
      >
        {isLoading ? (
          <>
            ...
            {t('ivCompat.matrix.querying')}
          </>
        ) : (
          <>
            <FlaskConical className="h-3 w-3 mr-1.5" />
            {t('ivCompat.matrix.generate', { count: drugs.length })}
          </>
        )}
      </Button>
        ...
        <>
          ...
          {incompatiblePairs.length > 0 && (
            <Alert variant="destructive" className="py-2 border-red-400">
              <AlertTriangle className="h-4 w-4" />
              <AlertTitle className="text-sm font-bold">
                {t('ivCompat.matrix.incompatibleSummary', { count: incompatiblePairs.length })}
              </AlertTitle>
              <AlertDescription className="text-xs mt-0.5 space-y-0.5">
                {incompatiblePairs.map((p) => (
                  <p key={`${p.drug1}-${p.drug2}`}>
                    {t('ivCompat.matrix.incompatiblePair', { a: p.drug1, b: p.drug2 })}
                  </p>
                ))}
              </AlertDescription>
            </Alert>
          ...
          <div className="flex flex-wrap gap-3 pt-1 text-xs text-muted-foreground">
            <span className="flex items-center gap-1">
              ...
              {t('ivCompat.matrix.legend.compatible')}
            </span>
            <span className="flex items-center gap-1">
              ...
              {t('ivCompat.matrix.legend.incompatible')}
            </span>
            <span className="flex items-center gap-1">
              ...
              {t('ivCompat.matrix.legend.noData')}
            </span>
          </div>
        </>
      )}
...
function QuickCheckPanel() {
  ...
  const handleCheck = useCallback(async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : t('ivCompat.quick.queryFailed');
    ...
    <div className="space-y-3">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end">
        <div className="flex-1 space-y-1">
          <label className="text-xs font-medium text-muted-foreground">{t('ivCompat.quick.labelA')}</label>
          <Input
            placeholder={t('ivCompat.quick.placeholderA')}
            value={state.drug1}
        ...
        <div className="flex-1 space-y-1">
          <label className="text-xs font-medium text-muted-foreground">{t('ivCompat.quick.labelB')}</label>
          <Input
            placeholder={t('ivCompat.quick.placeholderB')}
            value={state.drug2}
        ...
        >
          {state.loading ? (
            <>
              ...
              {t('ivCompat.quick.querying')}
            </>
          ) : (
            <>
              <FlaskConical className="h-3.5 w-3.5 mr-1.5" />
              {t('ivCompat.quick.submit')}
            </>
          )}
        </Button>
      </div>
      ...
      <CardHeader className="pb-2 space-y-1">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-base font-semibold leading-tight text-slate-800 dark:text-slate-200">{t('ivCompat.card.title')}</CardTitle>
          </div>
          {hasIVMeds && (
            <div className="flex gap-1">
              ...
              >
                {t('ivCompat.card.modeMatrix')}
              </Button>
            </div>
          )}
        </div>
        <CardDescription className="text-sm leading-tight">
          {t('ivCompat.card.description')}
        </CardDescription>
      </CardHeader>
