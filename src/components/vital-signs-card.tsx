  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-1">
        ...
        {canClick && <TrendingUp className="h-2.5 w-2.5 text-[#7f265b] opacity-70" />}
      </div>
      <div className="flex flex-1 flex-col items-center justify-center text-center">
        <span className={`text-lg font-semibold leading-none tracking-tight ${isAbnormal ? 'text-[#d97706]' : 'text-[#0f172a]'}`}>
          {displayValue}
        </span>
        {unit && <span className="mt-0.5 text-[8px] leading-tight text-[#64748b]">{unit}</span>}
      </div>
    </div>
  );
}
